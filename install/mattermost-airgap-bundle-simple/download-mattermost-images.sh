...
# Mattermost 10.12.0 이미지 다운로드 및 ECR 업로드 스크립트
...
echo "=== Mattermost $MATTERMOST_VERSION 이미지 다운로드 및 ECR 업로드 ==="
...
# 로컬 이미지 정리 (선택사항)
echo "로컬 이미지 정리 중..."
docker rmi mattermost/mattermost-team-edition:$MATTERMOST_VERSION
docker rmi $ECR_REGISTRY/$ECR_REPO:$MATTERMOST_VERSION
